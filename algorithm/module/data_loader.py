...
class json_file_data_loader(file_data_loader):
    ...
    def __init__(self,file_name,word_vec_file_name,rel2id_file_name,rel2id_file_name2,rel2id_file_name3,deepwalk_file_name,rel_rel2_filename,rel_rel3_filename,entity2id_file_name,mid_file_name,entity_type_file_name,mode,shuffle,sen_num_bag='all',max_length=config.model.max_length,case_sensitive=False,batch_size = config.model.batch_size):
        '''
        ...
        if(not self._load_processed_file(dataset)):
            ...
            with open(os.path.join(processed_data_dir,dataset+"_"+name_prefix+'_entpair2scope.json'),'w',encoding = 'utf8') as fw:
                json.dump(self.entpair2scope,fw,ensure_ascii=False)
            with open(os.path.join(processed_data_dir,dataset+"_"+name_prefix+'_relfact2scope.json'),'w',encoding='utf8') as fw:
                json.dump(self.relfact2scope,fw,ensure_ascii=False)

            np.save(os.path.join(processed_data_dir,dataset+"_"+word_vec_name_prefix+'_mat.npy'),self.word_vec_mat)
            with open(os.path.join(processed_data_dir,dataset+"_"+word_vec_name_prefix+'_word2id.json'),'w',encoding='utf8') as fw:
                json.dump(self.word2id,fw,ensure_ascii=False)
            with open(os.path.join(processed_data_dir,dataset+"_"+word_vec_name_prefix+'_id2word.json'),'w',encoding='utf8') as fw:
                json.dump(self.id2word,fw,ensure_ascii=False)
            with open(os.path.join(processed_data_dir,dataset+"_"+name_prefix+'_sentence.txt'),'w',encoding = 'utf8') as fw:
    ...
    def next_batch(self,batch_size):
        ...
        if self.mode == self.MODE_INSTANCE:
            ...
            for entity1 in batch_data['entity1']:
                ...
                batch_data['entity1_type_mask'].append([1.0 for j in range(len(entity1_type))]+[0 for j in range(len(self.entity_type_id) - len(entity1_type))])
                batch_data['entity1_type_length'].append(1.0*len(entity1_type))
                entity1_type = np.concatenate((entity1_type,np.array([0 for j in range(len(self.entity_type_id) - len(entity1_type))])))
            ...
            for entity2 in batch_data['entity2']:
                ...
                batch_data['entity2_type_mask'].append([1.0 for j in range(len(entity2_type))]+[0 for j in range(len(self.entity_type_id) - len(entity2_type))])
                batch_data['entity2_type_length'].append(1.0*len(entity2_type))
                entity2_type = np.concatenate((entity2_type,np.array([0 for j in range(len(self.entity_type_id) - len(entity2_type))])))
