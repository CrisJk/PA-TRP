...
class re_framework:
    ...
    def train(self, model, encoder, selector, entity_inf, model_name, ckpt_dir='./checkpoint', summary_dir='./summary',
        ...
        for gpu_id in gpu_list:
            with tf.device("/gpu:%d" % gpu_id):
                with tf.name_scope("gpu_%d" % gpu_id):
                    cur_model = model(self.train_data_loader, config.model.batch_size // gpu_nums,
                                      self.train_data_loader.max_length, encoder, selector, entity_inf, self.dataset)
        ...
        with open('loss.txt', 'w', encoding='utf8') as fw:
            for epoch in range(max_epoch):
                ...
                while True:
                    ...
                    try:
                        (iter_loss, iter_logit, _train_op, iter_interval,
                         iter_label), feed_dict = self.one_step_multi_models(tower_models, self.train_data_loader,
                                                                             [loss, train_logit, train_op, interval])
                    ...
                    if tot_not_na > 0:
                        sys.stdout.write(
                            "epoch %d step %d time %.2f | loss: %f, not NA accuracy: %f, accuracy: %f\r" % (
                                epoch, i, t, iter_loss, float(tot_not_na_correct) / tot_not_na,
                                float(tot_correct) / tot))
                        sys.stdout.flush()
                    i += 1
                ...
                if (epoch + 1) % test_epoch == 0:
                    ...
                    metric, cur_prec, cur_recall = test.test(model_name, self.test_data_loader, ckpt=None,
                                                             return_result=False, mode=config.model.test_level,
                                                             sess=self.sess)
